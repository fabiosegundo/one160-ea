...
public class DTNHost implements Comparable<DTNHost> {
	...
	*/
	public DTNHost(List<MessageListener> msgLs,
			...
			String groupId, int groupNr, List<NetworkInterface> interf,
			ModuleCommunicationBus comBus, 
			MovementModel mmProto, MessageRouter mRouterProto) {

		Settings s = new Settings(SimScenario.GROUP_NS+groupNr);
		s.setSecondaryNamespace(SimScenario.GROUP_NS);
		if (s.contains(EnergyModel.INIT_ENERGY_S)) {
			this.energy = new EnergyModel(s);
			this.energy = (energy != null ? energy.replicate() : null);
		} else {
		...
		if (s.contains(RANGE_COLOR_S)) {
			...
			else if (tmp.equals("blue")) {rangeColor = Color.BLUE;}
			else if (tmp.equals("yellow")) {rangeColor = Color.YELLOW;}
			else if (tmp.equals("cyan")) {rangeColor = Color.CYAN;}
			else if (tmp.equals("pink")) {rangeColor = Color.PINK;}
			else if (tmp.equals("orange")) {rangeColor = Color.ORANGE;}
			else if (tmp.equals("red")) {rangeColor = Color.RED;}
			else if (tmp.equals("gray")) {rangeColor = Color.GRAY;}
			else if (tmp.equals("veryMagenta")) {rangeColor = Color.MAGENTA;}
			else if (tmp.equals("lightGray")) {rangeColor = Color.LIGHT_GRAY;}
			else if (tmp.equals("darkGray")) {rangeColor = Color.DARK_GRAY;}
			else {rangeColor = Color.GREEN;}
		} else {
	...
	/**
	 * Returns the color to draw the range of this networks interfaces
	...
	public boolean isMovementActive() {
		//if (isNaN(getLocation().getX()) || isNaN(getLocation().getY())) return false;
		return this.movement.isActive();
	}
	
	/**
	 * Returns true if there is some node's radio active (false if none)
	 * @return true if this node's has some radio active (false if none)
	 */
	public boolean isAnyRadioActive() {
		if (isMovementActive())
			for (NetworkInterface ni : net)
				if (ni.isActive()) return true;
		return false;
	}
	...
	public boolean isRadioActiveByIfaceNo(int interfaceNo) {
		...
		try {
			...
			throw new SimError("isRadioActivate: No such interface: "+interfaceNo + 
					" at " + this);
		}
		return ni.isActive();
	}
	...
	/**
	 * Returns true if this node's radio is active (false if not)
	 * @param interfacetype type of the interface to verify
	 * @return true if this node's radio is active (false if not)
	 */
	public boolean isRadioActiveByIfaceType(String interfacetype) {
		/* TODO: make this work for multiple interfaces based on the same type in the same host */
		if (!isMovementActive()) 
			return false;
		for (NetworkInterface ni : net)
	...
	public boolean hasEnergy() {
		return energy == null || energy.getEnergy() != 0;
	}
	...
	/** 
	 * Tears down all connections for a interface in this host.
	 */
	private void tearDownAllConnectionsByIface(NetworkInterface i) {
	...
	private void tearDownAllConnections() {
		for (NetworkInterface i : net)
			tearDownAllConnectionsByIface(i);
	}
