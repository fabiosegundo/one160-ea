...
public class ExternalPathMovementReader {
	...
	private boolean convGpsCoord = true;
	...
	private ExternalPathMovementReader(String traceFilePath,
		...
		try {
			...
			this.normalize = lineScan.nextBoolean();
			this.convGpsCoord  = lineScan.nextBoolean();
		...
		while (line != null) {
			...
			while (traceScan.hasNext()) {
				...
				y = Double.parseDouble(dataPoint.substring(d1+1, d2));
				x = Double.parseDouble(dataPoint.substring(d2+1));
				ox = x; oy = y;

				if (this.convGpsCoord) {
	...
	/** 
	 * Returns the active time for the given start value.
	 * 
	 * @param times	active times of the node
	 * @param start start value to find	
	 * @return		the index of
	 */
	public int getIndexOfActiveStart(List<ActiveTime> times, double start) {
		int i=0;
		for (ActiveTime a: times) {
			if (a.start==start) return i;
			i++;
		}
		return -1;
	}
	 ...
	 * @return		the index of
	 */
	public int getIndexOfActiveEnd(List<ActiveTime> times, double end) {
		int i=0;
		for (ActiveTime a: times) {
			if (a.end==end) return i;
			i++;
		}
		return -1;
	}
